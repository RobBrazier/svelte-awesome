...
<main>
    <figure id="logo" on:mouseenter={toggle} on:mouseleave={toggle} on:click={change} on:keydown={change} title="{Object.keys(icon)[0]}">
    ...
    <p on:mouseenter={toggleIconOpen} on:mouseleave={toggleIconOpen}><Icon data={powerOff} style={`transform:rotate(${iconOpen ? 0 : -180}deg)`} scale={iconScale}/></p>
