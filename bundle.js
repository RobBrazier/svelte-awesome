(function () {
	...
	/* src/components/svg/Path.html generated by Svelte v2.13.3 */
	...
	function create_main_fragment(component, ctx) {
		var path, path_key_value, current;

		return {
			c() {
				path = createSvgElement("path");
				setAttribute(path, "key", path_key_value = "path-" + ctx.id);
			},

			m(target, anchor) {
				insert(target, path, anchor);
				component.refs.path = path;
				current = true;
			},

			p(changed, ctx) {
				if ((changed.id) && path_key_value !== (path_key_value = "path-" + ctx.id)) {
					setAttribute(path, "key", path_key_value);
				}
			},
			...
			d(detach) {
				...
				if (component.refs.path === path) component.refs.path = null;
			}
		};
	}
	...
	assign(Path.prototype, proto);

	/* src/components/svg/Polygon.html generated by Svelte v2.13.3 */
	...
	function onupdate$1({ changed }) {
	  if (changed.data) {
	    const { data } = this.get();
	    const { polygon } = this.refs;
	    Object.keys(data).forEach((key) => {
	...
	function create_main_fragment$1(component, ctx) {
		var polygon, polygon_key_value, current;

		return {
			c() {
				polygon = createSvgElement("polygon");
				setAttribute(polygon, "key", polygon_key_value = "polygon-" + ctx.id);
			},

			m(target, anchor) {
				insert(target, polygon, anchor);
				component.refs.polygon = polygon;
				current = true;
			},

			p(changed, ctx) {
				if ((changed.id) && polygon_key_value !== (polygon_key_value = "polygon-" + ctx.id)) {
					setAttribute(polygon, "key", polygon_key_value);
				}
			},
			...
			d(detach) {
				...
				if (component.refs.polygon === polygon) component.refs.polygon = null;
			}
		};
	}
	...
	assign(Polygon.prototype, proto);

	/* src/components/svg/Raw.html generated by Svelte v2.13.3 */
	...
	function create_main_fragment$2(component, ctx) {
		...
		return {
			...
			m(target, anchor) {
				...
				append(g, raw_1_after);
				raw_1_after.insertAdjacentHTML("beforebegin", ctx.raw);
				current = true;
			},
	...
	assign(Raw.prototype, proto);

	Raw.prototype._recompute = function _recompute(changed, state) {
		if (this._differs(state.raw, (state.raw = raw(exclude(state, "raw"))))) changed.raw = true;
	};

	/* src/components/svg/Svg.html generated by Svelte v2.13.3 */
	...
	function create_main_fragment$3(component, ctx) {
		var svg, slot_content_default = component._slotted.default, slot_content_default_after, svg_class_value, svg_role_value, current;

		return {
			c() {
				svg = createSvgElement("svg");
				setAttribute(svg, "version", "1.1");
				setAttribute(svg, "class", svg_class_value = "" + ctx.klass + " " + ctx.class + " svelte-1dof0an");
				setAttribute(svg, "width", ctx.width);
				setAttribute(svg, "height", ctx.height);
				setAttribute(svg, "role", svg_role_value = ctx.label ? 'img' : 'presentation');
				setAttribute(svg, "viewBox", ctx.box);
			},
			...
			m(target, anchor) {
				...
				if (slot_content_default) {
					append(svg, slot_content_default);
					append(svg, slot_content_default_after || (slot_content_default_after = createComment()));
				}
			...
			p(changed, ctx) {
				if ((changed.klass || changed.class) && svg_class_value !== (svg_class_value = "" + ctx.klass + " " + ctx.class + " svelte-1dof0an")) {
				...
				if ((changed.label) && svg_role_value !== (svg_role_value = ctx.label ? 'img' : 'presentation')) {
					setAttribute(svg, "role", svg_role_value);
				}
	...
	function Svg(options) {
		...
		this._recompute({ spin: 1, flip: 1, inverse: 1, pulse: 1 }, this._state);
		this._intro = !!options.intro;
	...
	Svg.prototype._recompute = function _recompute(changed, state) {
		if (changed.spin || changed.flip || changed.inverse || changed.pulse) {
			if (this._differs(state.klass, (state.klass = klass(state)))) changed.klass = true;
		}
	};

	/* src/components/Icon.html generated by Svelte v2.13.3 */

	const icons = {};

	function normalisedScale({ scale, outerScale }) {
	  let numScale = 1;
	...
	function box({ icon, width, height }) {
	  if (icon) {
	...
	function style({ normalisedScale }) {
	...
	function create_main_fragment$4(component, ctx) {
		var text, slot_content_default = component._slotted.default, slot_content_default_before, slot_content_default_after, if_block_anchor, text_2, current;

		var if_block = (ctx.icon) && create_if_block(component, ctx);

		var svg_initial_data = {
		 	class: ctx.class,
		 	label: ctx.label,
		 	width: ctx.width,
		 	height: ctx.height,
		...
		var svg = new Svg({
			...
			data: svg_initial_data
		});

		component.refs.icon = svg;
		...
		return {
			...
			m(target, anchor) {
				append(svg._slotted.default, text);
				if (!slot_content_default) {
					if (if_block) if_block.m(svg._slotted.default, null);
					append(svg._slotted.default, if_block_anchor);
				}

				else {
					append(svg._slotted.default, slot_content_default_before || (slot_content_default_before = createComment()));
					append(svg._slotted.default, slot_content_default);
					append(svg._slotted.default, slot_content_default_after || (slot_content_default_after = createComment()));
				}

				append(svg._slotted.default, text_2);
				svg._mount(target, anchor);
				current = true;
			},
			...
			p(changed, ctx) {
				...
				var svg_changes = {};
				if (changed.class) svg_changes.class = ctx.class;
				...
				if (changed.style) svg_changes.style = ctx.style;
			...
			o(outrocallback) {
				...
				if (svg) svg._fragment.o(outrocallback);
				current = false;
			},
			...
			d(detach) {
				...
				svg.destroy(detach);
				if (component.refs.icon === svg) component.refs.icon = null;
			}
		};
	}

	// (6:8) {#each icon.paths as path, i}
	function create_each_block(component, ctx) {
		var current;

		var path_initial_data = { id: ctx.i, data: ctx.path };
		var path = new Path({
			root: component.root,
			store: component.store,
			data: path_initial_data
		});
		...
		return {
			...
			m(target, anchor) {
				path._mount(target, anchor);
				current = true;
			},

			p(changed, ctx) {
				var path_changes = {};
				if (changed.icon) path_changes.data = ctx.path;
				path._set(path_changes);
			},
			...
			o(outrocallback) {
				...
				if (path) path._fragment.o(outrocallback);
				current = false;
			},
	...
	// (11:8) {#each icon.polygons as polygon, i}
	function create_each_block_1(component, ctx) {
		var current;

		var polygon_initial_data = { id: ctx.i, data: ctx.polygon };
		var polygon = new Polygon({
			root: component.root,
			store: component.store,
			data: polygon_initial_data
		});
		...
		return {
			...
			m(target, anchor) {
				polygon._mount(target, anchor);
				current = true;
			},

			p(changed, ctx) {
				var polygon_changes = {};
				if (changed.icon) polygon_changes.data = ctx.polygon;
				polygon._set(polygon_changes);
			},
			...
			o(outrocallback) {
				...
				if (polygon) polygon._fragment.o(outrocallback);
				current = false;
			},

			d(detach) {
				polygon.destroy(detach);
			}
		};
	}

	// (10:6) {#if icon.polygons}
	function create_if_block_2(component, ctx) {
		var each_anchor, current;

		var each_value_1 = ctx.icon.polygons;
		...
		return {
			...
			p(changed, ctx) {
				if (changed.icon) {
					each_value_1 = ctx.icon.polygons;
	...
	function create_if_block_3(component, ctx) {
		...
		var raw_initial_data = { icon: ctx.icon };
		var raw = new Raw({
			root: component.root,
			store: component.store,
			data: raw_initial_data
		});
		...
		return {
			...
			m(target, anchor) {
				raw._mount(target, anchor);
				current = true;
			},

			p(changed, ctx) {
				var raw_changes = {};
				if (changed.icon) raw_changes.icon = ctx.icon;
				raw._set(raw_changes);
			},
			...
			o(outrocallback) {
				...
				if (raw) raw._fragment.o(outrocallback);
				current = false;
			},
	...
	function create_if_block(component, ctx) {
		var text, text_1, if_block_2_anchor, current;

		var if_block = (ctx.icon.paths) && create_if_block_1(component, ctx);

		var if_block_1 = (ctx.icon.polygons) && create_if_block_2(component, ctx);

		var if_block_2 = (ctx.icon.raw) && create_if_block_3(component, ctx);
		...
		return {
			...
			m(target, anchor) {
				...
				insert(target, if_block_2_anchor, anchor);
				current = true;
			},
			...
			p(changed, ctx) {
				...
				if (ctx.icon.raw) {
					...
					if_block_2.i(if_block_2_anchor.parentNode, if_block_2_anchor);
				} else if (if_block_2) {
	...
	function Icon(options) {
		...
		this._recompute({ scale: 1, outerScale: 1, name: 1, icon: 1, childrenWidth: 1, ratio: 1, normalisedScale: 1, childrenHeight: 1, width: 1, height: 1 }, this._state);
		this._intro = !!options.intro;
	...
	Icon.prototype._recompute = function _recompute(changed, state) {
		if (changed.scale || changed.outerScale) {
			if (this._differs(state.normalisedScale, (state.normalisedScale = normalisedScale(state)))) changed.normalisedScale = true;
		}
		...
		if (changed.childrenWidth || changed.icon || changed.ratio || changed.normalisedScale) {
			if (this._differs(state.width, (state.width = width(state)))) changed.width = true;
		}

		if (changed.childrenHeight || changed.icon || changed.ratio || changed.normalisedScale) {
			if (this._differs(state.height, (state.height = height(state)))) changed.height = true;
		}

		if (changed.icon || changed.width || changed.height) {
			if (this._differs(state.box, (state.box = box(state)))) changed.box = true;
		}

		if (changed.normalisedScale) {
	...
	/* example/App.html generated by Svelte v2.13.3 */

	var keys = Object.keys(Icon.icons);
	...
	function oncreate$1() {
	  ...
	  setInterval(function () {
	    if (this$1.get().running) {
	      this$1.change();
	    }
	  }, 200);
	}
	function create_main_fragment$5(component, ctx) {
		var main, figure, icon_updating = {}, text_1, h1, text_3, p, text_5, h2, text_7, p_1, text_8, figure_1, text_14, h2_1, text_16, p_2, text_17, figure_2, text_27, h2_2, text_29, p_3, text_30, figure_3, text_38, h2_3, text_40, p_4, text_41, figure_4, text_49, h2_4, text_51, p_5, text_52, figure_5, text_62, h2_5, text_64, p_6, text_66, p_7, text_67, figure_6, text_77, h2_6, text_79, p_8, text_81, p_9, text_82, text_83, text_84, text_86, figure_7, text_108, h2_7, text_110, p_10, text_112, p_11, text_113, figure_8, text_119, h2_8, text_121, p_12, text_123, p_13, text_124, text_125, text_126, p_14, text_130, footer, current;

		var icon_initial_data = { scale: "4" };
		if (ctx.logo !== void 0) {
			icon_initial_data.name = ctx.logo;
			icon_updating.name = true;
		}
		var icon = new Icon({
			root: component.root,
			store: component.store,
			...
			_bind: function _bind(changed, childState) {
				var newState = {};
				if (!icon_updating.name && changed.name) {
					newState.logo = childState.name;
				}
		...
		var icon_1_initial_data = { name: "flag" };
		var icon_1 = new Icon({
			root: component.root,
			store: component.store,
			data: icon_1_initial_data
		});

		var icon_2_initial_data = { name: "language", scale: "3" };
		var icon_2 = new Icon({
			root: component.root,
			store: component.store,
			data: icon_2_initial_data
		});

		var icon_3_initial_data = { name: "refresh", spin: true };
		var icon_3 = new Icon({
			root: component.root,
			store: component.store,
			data: icon_3_initial_data
		});

		var icon_4_initial_data = { name: "spinner", pulse: true };
		var icon_4 = new Icon({
			root: component.root,
			store: component.store,
			data: icon_4_initial_data
		});

		var icon_5_initial_data = { name: "signal", flip: "horizontal" };
		var icon_5 = new Icon({
			root: component.root,
			store: component.store,
			data: icon_5_initial_data
		});

		var icon_6_initial_data = { name: "code", label: "Source Code" };
		var icon_6 = new Icon({
			root: component.root,
			store: component.store,
			data: icon_6_initial_data
		});

		var icon_8_initial_data = { name: "camera" };
		var icon_8 = new Icon({
			root: component.root,
			store: component.store,
			data: icon_8_initial_data
		});

		var icon_9_initial_data = { name: "ban", scale: "2", class: "alert" };
		var icon_9 = new Icon({
			root: component.root,
			store: component.store,
			data: icon_9_initial_data
		});

		var icon_7_initial_data = { label: "No Photo" };
		var icon_7 = new Icon({
			root: component.root,
			store: component.store,
			slots: { default: createFragment() },
			data: icon_7_initial_data
		});

		var icon_10_initial_data = { name: "baidu" };
		var icon_10 = new Icon({
			root: component.root,
			store: component.store,
			data: icon_10_initial_data
		});

		var icon_11_initial_data = { name: "webpack" };
		var icon_11 = new Icon({
			root: component.root,
			store: component.store,
			data: icon_11_initial_data
		});

		var icon_12_initial_data = { name: "vue" };
		var icon_12 = new Icon({
			root: component.root,
			store: component.store,
			data: icon_12_initial_data
		});

		var icon_13_initial_data = { name: "html5-c" };
		var icon_13 = new Icon({
			root: component.root,
			store: component.store,
			data: icon_13_initial_data
		});
		...
		return {
			c: function c() {
				...
				text_1 = createText("\n    ");
				h1 = createElement("h1");
				h1.innerHTML = "<a href=\"https://github.com/RobBrazier/svelte-awesome\">Svelte-Awesome</a>";
				text_3 = createText("\n    ");
				p = createElement("p");
				p.textContent = "Font Awesome component for Svelte.js, using inline SVG.";
				text_5 = createText("\n\n    ");
				h2 = createElement("h2");
				h2.textContent = "Basic";
				text_7 = createText("\n    ");
				p_1 = createElement("p");
				icon_1._fragment.c();
				text_8 = createText("\n    ");
				figure_1 = createElement("figure");
				figure_1.innerHTML = "<pre><code>&lt;Icon <span class=\"attr\">name</span>=<span class=\"val\">\"flag\"</span>&gt;&lt;/Icon&gt;</code></pre>";
				text_14 = createText("\n\n    ");
				h2_1 = createElement("h2");
				h2_1.textContent = "Scale";
				text_16 = createText("\n    ");
				p_2 = createElement("p");
				icon_2._fragment.c();
				text_17 = createText("\n    ");
				figure_2 = createElement("figure");
				figure_2.innerHTML = "<pre><code>&lt;Icon <span class=\"attr\">name</span>=<span class=\"val\">\"language\"</span> <span class=\"attr\">scale</span>=<span class=\"val\">\"3\"</span>&gt;&lt;/Icon&gt;</code></pre>";
				text_27 = createText("\n\n    ");
				h2_2 = createElement("h2");
				h2_2.textContent = "Spin";
				text_29 = createText("\n    ");
				p_3 = createElement("p");
				icon_3._fragment.c();
				text_30 = createText("\n    ");
				figure_3 = createElement("figure");
				figure_3.innerHTML = "<pre><code>&lt;Icon <span class=\"attr\">name</span>=<span class=\"val\">\"refresh\"</span> <span class=\"attr\">spin</span>&gt;&lt;/Icon&gt;</code></pre>";
				text_38 = createText("\n\n    ");
				h2_3 = createElement("h2");
				h2_3.textContent = "Pulse";
				text_40 = createText("\n    ");
				p_4 = createElement("p");
				icon_4._fragment.c();
				text_41 = createText("\n    ");
				figure_4 = createElement("figure");
				figure_4.innerHTML = "<pre><code>&lt;Icon <span class=\"attr\">name</span>=<span class=\"val\">\"spinner\"</span> <span class=\"attr\">pulse</span>&gt;&lt;/Icon&gt;</code></pre>";
				text_49 = createText("\n\n    ");
				h2_4 = createElement("h2");
				h2_4.textContent = "Flip";
				text_51 = createText("\n    ");
				p_5 = createElement("p");
				icon_5._fragment.c();
				text_52 = createText("\n    ");
				figure_5 = createElement("figure");
				figure_5.innerHTML = "<pre><code>&lt;Icon <span class=\"attr\">name</span>=<span class=\"val\">\"signal\"</span> <span class=\"attr\">flip</span>=<span class=\"val\">\"horizontal\"</span>&gt;&lt;/Icon&gt;</code></pre>";
				text_62 = createText("\n\n    ");
				h2_5 = createElement("h2");
				h2_5.textContent = "Label";
				text_64 = createText("\n    ");
				p_6 = createElement("p");
				p_6.innerHTML = "<small>Accessible for screen readers, etc.</small>";
				text_66 = createText("\n    ");
				p_7 = createElement("p");
				icon_6._fragment.c();
				text_67 = createText("\n    ");
				figure_6 = createElement("figure");
				figure_6.innerHTML = "<pre><code>&lt;Icon <span class=\"attr\">name</span>=<span class=\"val\">\"code\"</span> <span class=\"attr\">label</span>=<span class=\"val\">\"Source Code\"</span>&gt;&lt;/Icon&gt;</code></pre>";
				text_77 = createText("\n\n    ");
				h2_6 = createElement("h2");
				h2_6.textContent = "Stacked icons";
				text_79 = createText("\n    ");
				p_8 = createElement("p");
				p_8.innerHTML = "<small>Use stacked icons like in FontAwesome. Even more powerful.</small>";
				text_81 = createText("\n    ");
				p_9 = createElement("p");
				text_82 = createText("\n        ");
				icon_8._fragment.c();
				...
				icon_7._fragment.c();
				text_86 = createText("\n    ");
				figure_7 = createElement("figure");
				figure_7.innerHTML = "<pre><code>&lt;Icon <span class=\"attr\">label</span>=<span class=\"val\">\"No Photos\"</span>&gt;\n  &lt;Icon <span class=\"attr\">name</span>=<span class=\"val\">\"camera\"</span>&gt;&lt;/Icon&gt;\n  &lt;Icon <span class=\"attr\">name</span>=<span class=\"val\">\"ban\"</span> <span class=\"attr\">scale</span>=<span class=\"val\">\"2\"</span> <span class=\"attr\">class</span>=<span class=\"val\">\"alert\"</span>&gt;&lt;/Icon&gt;\n&lt;/Icon&gt;</code></pre>";
				text_108 = createText("\n\n    ");
				h2_7 = createElement("h2");
				h2_7.textContent = "Custom icons";
				text_110 = createText("\n    ");
				p_10 = createElement("p");
				p_10.innerHTML = "<small>You can register your own icons.</small>";
				text_112 = createText("\n    ");
				p_11 = createElement("p");
				icon_10._fragment.c();
				text_113 = createText("\n    ");
				figure_8 = createElement("figure");
				figure_8.innerHTML = "<pre><code>&lt;Icon <span class=\"attr\">name</span>=<span class=\"val\">\"baidu\"</span>&gt;&lt;/Icon&gt;</code></pre>";
				text_119 = createText("\n\n    ");
				h2_8 = createElement("h2");
				h2_8.textContent = "Multi-color icons";
				text_121 = createText("\n    ");
				p_12 = createElement("p");
				p_12.innerHTML = "<small>Register icons in more advanced ways to unleash the full power of SVG.</small>";
				text_123 = createText("\n    ");
				p_13 = createElement("p");
				icon_11._fragment.c();
				text_124 = createText(" ");
				icon_12._fragment.c();
				text_125 = createText(" ");
				icon_13._fragment.c();
				text_126 = createText("\n    ");
				p_14 = createElement("p");
				p_14.innerHTML = "See <a href=\"https://github.com/Justineo/vue-awesome#more-advanced-cases\">readme</a> for further instructions.";
				text_130 = createText("\n\n    ");
				footer = createElement("footer");
				footer.innerHTML = "<a href=\"//github.com/RobBrazier\">@RobBrazier</a>|<a href=\"//github.com/RobBrazier/svelte-awesome/blob/master/LICENSE\">MIT License</a>|<a href=\"//github.com/RobBrazier/svelte-awesome\">View on GitHub</a>";
				addListener(figure, "mouseenter", mouseenter_handler);
				addListener(figure, "mouseleave", mouseleave_handler);
				addListener(figure, "click", click_handler);
				figure.id = "logo";
				figure.title = ctx.logo;
				p.className = "desc";
			},
			...
			m: function m(target, anchor) {
				...
				append(main, text_7);
				append(main, p_1);
				icon_1._mount(p_1, null);
				append(main, text_8);
				append(main, figure_1);
				append(main, text_14);
				append(main, h2_1);
				append(main, text_16);
				append(main, p_2);
				icon_2._mount(p_2, null);
				append(main, text_17);
				append(main, figure_2);
				append(main, text_27);
				append(main, h2_2);
				append(main, text_29);
				append(main, p_3);
				icon_3._mount(p_3, null);
				append(main, text_30);
				append(main, figure_3);
				append(main, text_38);
				append(main, h2_3);
				append(main, text_40);
				append(main, p_4);
				icon_4._mount(p_4, null);
				append(main, text_41);
				append(main, figure_4);
				append(main, text_49);
				append(main, h2_4);
				append(main, text_51);
				append(main, p_5);
				icon_5._mount(p_5, null);
				append(main, text_52);
				append(main, figure_5);
				append(main, text_62);
				append(main, h2_5);
				append(main, text_64);
				append(main, p_6);
				append(main, text_66);
				append(main, p_7);
				icon_6._mount(p_7, null);
				append(main, text_67);
				append(main, figure_6);
				append(main, text_77);
				append(main, h2_6);
				append(main, text_79);
				append(main, p_8);
				append(main, text_81);
				append(main, p_9);
				append(icon_7._slotted.default, text_82);
				icon_8._mount(icon_7._slotted.default, null);
				append(icon_7._slotted.default, text_83);
				icon_9._mount(icon_7._slotted.default, null);
				append(icon_7._slotted.default, text_84);
				icon_7._mount(p_9, null);
				append(main, text_86);
				append(main, figure_7);
				append(main, text_108);
				append(main, h2_7);
				append(main, text_110);
				append(main, p_10);
				append(main, text_112);
				append(main, p_11);
				icon_10._mount(p_11, null);
				append(main, text_113);
				append(main, figure_8);
				append(main, text_119);
				append(main, h2_8);
				append(main, text_121);
				append(main, p_12);
				append(main, text_123);
				append(main, p_13);
				icon_11._mount(p_13, null);
				append(p_13, text_124);
				icon_12._mount(p_13, null);
				append(p_13, text_125);
				icon_13._mount(p_13, null);
				append(main, text_126);
				append(main, p_14);
				append(main, text_130);
				append(main, footer);
				current = true;
			},
			...
			p: function p(changed, _ctx) {
				ctx = _ctx;
				var icon_changes = {};
				if (!icon_updating.name && changed.logo) {
					icon_changes.name = ctx.logo;
					icon_updating.name = ctx.logo !== void 0;
				}
			...
			o: function o(outrocallback) {
				...
				if (icon) { icon._fragment.o(outrocallback); }
				if (icon_1) { icon_1._fragment.o(outrocallback); }
				if (icon_2) { icon_2._fragment.o(outrocallback); }
				if (icon_3) { icon_3._fragment.o(outrocallback); }
				if (icon_4) { icon_4._fragment.o(outrocallback); }
				if (icon_5) { icon_5._fragment.o(outrocallback); }
				if (icon_6) { icon_6._fragment.o(outrocallback); }
				if (icon_8) { icon_8._fragment.o(outrocallback); }
				if (icon_9) { icon_9._fragment.o(outrocallback); }
				if (icon_7) { icon_7._fragment.o(outrocallback); }
				if (icon_10) { icon_10._fragment.o(outrocallback); }
				if (icon_11) { icon_11._fragment.o(outrocallback); }
				if (icon_12) { icon_12._fragment.o(outrocallback); }
				if (icon_13) { icon_13._fragment.o(outrocallback); }
				current = false;
			},
			...
			d: function d(detach) {
				...
				icon.destroy();
				removeListener(figure, "mouseenter", mouseenter_handler);
				removeListener(figure, "mouseleave", mouseleave_handler);
				removeListener(figure, "click", click_handler);
				icon_1.destroy();
				icon_2.destroy();
				icon_3.destroy();
				icon_4.destroy();
